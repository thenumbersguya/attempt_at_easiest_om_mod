...
function incrementyverseRender() {
  ...
  let hardyDisplay = "H<sub>" + OPord + "</sub><invisible>l</invisible>(2)"
  if (game.colors == 1) {
...
function realDisplayHugeOrd2(x) {
  if (x.gte(ordThreshData["hyperoperational cutoff"])) {
    return "Ω<sub>Ω</sub>"
  }
  if (x.gte(ordThreshData["buchholz e(W2+1)"])&&game.buchholz != 0) {
    return "Ω<sub>ω+1</sub>"
  }
  if (x.gte(ordThreshData["madore e(W2+1)"])&&game.buchholz == 0) {
    return "ε<sub>Ω<sub>ω</sub>+1</sub>"
  }
  if (x.gte(2**2)) {
    return "Ω<sub>ω</sub>"
  }
  if (x.gte(2)) {
    return displayOrd(x.toNumber(),2,0,game.maxOrdLength.less-game.maxOrdLength.more).split("ω").join("Ω")
  }
  return x.toString()
}
