...
function infinity(manmade = 0) {
  if (game.canInf && !game.incrementyverse) {
    if (calcOrdPoints(game.ord, game.base, game.over) >= 1e265) {
      ...
      if (game.base == 2) {
        if (game.OP >= 6e270 && game.OP < 3.6e271) game.OP = 3.6e271
        if (game.OP >= 3.7e271 && game.OP < 4e271) game.OP = 4e271
        if (game.OP >= 4.2e271 && game.OP < 7.2e271) game.OP = 7.2e271
        if (game.OP >= 7.3e271) game.OP = BHO * getSingLevel()
      }
    } else {
