...
const slugMile = [1e300, 1e300, 1e300, 1e300, 1e300, 1e300, -1];
let totalMult = 1;
let buptotalMute = 1;
...
/* eslint-enable */
const iupCosts = [1e5, 1e3, 1e9, 0, 0, 0, 0, 0, 0];
const dupCosts = [
  5,
  1000,
  9,
...
const baselessMile = [0,0,0,0,Infinity]
let ordColor = "no";
...
function loop(unadjusted, off = 0) {
  ...
  if (game.incrementyverse == 1) {
    incrementyverseLoop(Math.max(0,unadjusted * 1000),off)
    game.lastTick = Date.now();
    return
  }
  if (game.boostUnlock) factorShiftCosts[8] = Infinity
  let ms=Math.max(0,unadjusted * ((game.challenge == 6 || game.challenge == 7) && game.dynamic > 0 ? 1 : 1000)); // x1000 here! (c6+c7 fix)
  ...
  if ((game.OP > BHO * getSingLevel()) || (game.base == 2 && game.OP >= BHO)) game.OP = BHO * getSingLevel();
  if ((game.ord > BHO * getSingLevel()) || (game.base == 2 && game.ord >= BHO)) game.ord = BHO * getSingLevel();
  if (getBaseless()>=3) {
  ...
  if (
    ...
    !((game.ord >= 3 ** 27 && game.base == 3) || (game.ord >= 2 ** 2 && game.base == 2)) //&&
    //!(chal8Tip)
  ) {
...
function render() {
  ...
  if (ordLevel == 154) {
    get("nextPsiLevel").innerHTML = displayHugeOrd(ordThreshData["buchholz e(W2+1)"],game.base)
  } else {
  ...
  get("ordinalPointsDisplay").innerHTML =
    `You have ${fbvps>=10&&game.OP>=1e270?beautify(calcOPPS()*game.msint/1000):beautify2(game.OP)} Ordinal Points`;
  ...
  get("factorShift").innerHTML =
    ...
    (inChal(5) && game.factorShifts >= 2
      ? "Infinity"
      : game.factorShifts == 7
      ? (game.factorShifts >= 9 - game.boostUnlock)
        ? "Infinity"
        : ((getOCComp(4) == 0 && game.factorShifts == 7) ? "Graham's number (g<sub>ψ(Ω<sup>Ω</sup>ω)</sub> (10))" : beautify(getFSCost(game.factorShifts)))
  ...
  get("darkManifoldAmount").textContent =
    beautifyEN(game.darkManifolds,3,true) + (game.sing.dm===0||(game.sfBought.includes(11))?"":"-" + beautifyEN(getDMSacrafice(),3))
...
function displayOrd(
  ...
  if (ord == Infinity) {
    ...
    return output.join("");
  } else if ((getSingLevel() == 1||game.base == 2)&&ord==BHO) {
    if (ordColor == "no") ordColor = HSL(40 * 8);
    return colour == 1 ? color("BHO", ["BHO"], HSL(80 * 4)) : "BHO";
  } else if (game.base == 2&&ord>BHO&&ord < BHO * (ordMarks[0].length - 39)) {
    if (ordColor == "no") ordColor = HSL(40 * 8);
    return colour == 1 ? color("ψ(Ω<sub>ω</sub>)", ["ψ(Ω<sub>ω</sub>)"], HSL(80 * 4)) : "ψ(Ω<sub>ω</sub>)";
...
function beautify2(number) {
  let num = number
  if (game.base == 2 && num >= 10**265 && num < Infinity) {
    //4,5,36,40,41,72,BHO
    if (num >= 6.001e270 && num <= 3.6001e271) num = 6e270 // display 5e270 instead of 3.5e271
    if (num >= 3.7001e271 && num <= 4.0001e271) num = 3.7e271 // display 3.6e271 instead of 3.9e271
    if (num >= 4.2001e271 && num <= 7.2001e271) num = 4.2e271 // display 4.1e271 instead of 7.1e271
  }
  return beautify(num)
}
...
function calcOrdPoints(ord = game.ord, base = game.base, over = game.over) {
  if (!((ord > 3 ** 27 && base == 3) || (ord > 2 ** 2 && base == 2))) {
    if (ord < base) {
      return ord + over;
    } else {
      let tempvar = Math.floor(Math.log(ord + 0.1) / Math.log(base));
...
function maxFactors() {
  if (!inChal(2)) {
    if (game.factors.length >= 7 && game.OP >= 1e257) {
      ...
      if (game.factorShifts >= 8) game.factors.push(1);
    } else {
