...
function calcFactorShiftTime(n) {
  return Math.max(
    ...
    (100 / calcOPPS(n - 1)) * (game.leastBoost <= 1.5 ? 1 : 1) +
      (calcBase(n-1)==2?2**2:calcBase(n-1)==3?3**27:OPtoOrd(getFSCost(n-1),calcBase(n-1))) /
...
function calcFactorBoostTime() {
  ...
  return (
    Math.max(
      ...
      fbt + EN((getOCComp(4) >= 1 ? 108 : 117) / speed).toNumber() // (BHO / 1e270 / speed) * 3 ** (getSingLevel() - 1)
    ) / getFBmult()
  );
}
...
function addBase2Ord(ord, num) {
  //0,1,2,3,4,5,6->36,37->40,41,42->72,73->BHO
  let ord1 = ord / (10**270)
  let ord2 = ord1
  if (ord1 >= 6 && ord1 < 36) ord2 = 6
  if (ord1 >= 36 && ord1 < 37) ord2 = 6 + (ord1 - 36)
  if (ord1 >= 37 && ord1 < 40) ord2 = 7
  if (ord1 >= 40 && ord1 < 42) ord2 = 7 + (ord1 - 40)
  if (ord1 >= 42 && ord1 < 72) ord2 = 9
  if (ord1 >= 72 && ord1 < 73) ord2 = 9 + (ord1 - 72)
  if (ord1 >= 73 && ord < BHO * getSingLevel()) ord2 = 10
  if (ord >= BHO * getSingLevel()) ord2 = 10
  ord2 += num
  if (ord2 < 6) return ord2 * (10**270)
  if (ord2 >= 6 && ord2 < 7) return 3.6e271 + ((ord2 - 6) * (10**270))
  if (ord2 >= 7 && ord2 < 9) return 4e271 + ((ord2 - 7) * (10**270))
  if (ord2 >= 9 && ord2 < 10) return 7.2e271 + ((ord2 - 9) * (10**270))
  if (ord2 >= 10) return BHO * getSingLevel()
}
...
function increaseOrdTier2(x) {
  ...
  if (
    ...
    if (game.base == 2) {
      game.OP = addBase2Ord(game.OP, bupCom / 1e270)
      game.ord = addBase2Ord(game.ord, bupCom / 1e270)
